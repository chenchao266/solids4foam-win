...
namespace Foam
{
    ...
    defineTypeNameAndDebug(fv::pointGaussLsDivSigmaScheme, 1);
    fv::divSigmaScheme::addIstreamConstructorToTable
    <
        fv::pointGaussLsDivSigmaScheme
    > addPointGaussLsDivSigmaSchemeIstreamConstructorToTable_;
...
void pointGaussLsDivSigmaScheme::insertCoeffsNorm
(
...
)
{
    ...
    forAll(mesh.boundaryMesh(), patchI)
    {
        ...
        else if (patchType == processorPolyPatch::typeName)
        {
            ...
            //     volToPointInterp_.pointProcFaces();
...
void pointGaussLsDivSigmaScheme::insertCoeffsBc
(
    const solidPolyMesh& solidMesh,
    const surfaceScalarField& muf,
    const surfaceScalarField& lambdaf,
...
)
{
    ...
    {
        const word pType = U.boundaryField()[patchI].type();

        if (pType == "solidSymmetry" || pType == "symmetry")
        {
            FatalErrorIn("insertCoeffsBc")
                << "symmetry disabled" << abort(FatalError);
        }
        else if
        (
...
pointGaussLsDivSigmaScheme::pointGaussLsDivSigmaScheme
(
    const fvMesh& mesh,
    Istream& is
)
:
    divSigmaScheme(mesh, is)
{}


// * * * * * * * * * Public member functions  * * * * * * * * * * * * * * * * //

tmp<BlockLduMatrix<vector> > pointGaussLsDivSigmaScheme::fvmDivSigma
(
    const solidPolyMesh& solidMesh,
    const surfaceScalarField& muf,
    const surfaceScalarField& lambdaf,
...
)
{
    ...
    {
        FatalErrorIn("pointGaussLsDivSigmaScheme::fvmDivSigma")
    ...
    insertCoeffsNorm(solidMesh, muf, lambdaf, U, blockB, blockM);

    // Insert coeffs due to tangential derivative terms
    insertCoeffsTang(solidMesh, muf, lambdaf, U, blockB, blockM);

    // Insert coeffs due to the boundary conditions
    insertCoeffsBc(solidMesh, muf, lambdaf, U, blockB, blockM);
