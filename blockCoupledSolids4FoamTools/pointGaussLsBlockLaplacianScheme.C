...
namespace Foam
{
    ...
    defineTypeNameAndDebug(fv::pointGaussLsBlockLaplacianScheme, 0);
    fv::blockLaplacian::addIstreamConstructorToTable
    <
        fv::pointGaussLsBlockLaplacianScheme
    > addPointGaussLsBlockLaplacianSchemeIstreamConstructorToTable_;
...
void pointGaussLsBlockLaplacianScheme::insertCoeffsNorm
(
...
pointGaussLsBlockLaplacianScheme::pointGaussLsBlockLaplacianScheme
(
    const fvMesh& mesh,
    Istream& is
)
:
    blockLaplacian(mesh, is)
{}


// * * * * * * * * * Public member functions  * * * * * * * * * * * * * * * * //

tmp<BlockLduMatrix<vector> > pointGaussLsBlockLaplacianScheme::fvmBlockLaplacian
(
    const solidPolyMesh& solidMesh,
    const surfaceScalarField& muf,
...
)
{
    ...
    {
        FatalErrorIn("pointGaussLsBlockLaplacianScheme::fvmDivSigma")
