...
void newLeastSquaresVolPointInterpolation::makePointFaces() const
{
    ...
    forAll(pointBndFaces, pointI)
    {
        ...
        // ZT: this part is not necessary any more since problem is solve
        // by lookin for duplicate cell centres below
        // FIX: Ph 2-11-21
...
//         }
...
#endif // OpenFOAM assumes no global points
        {
            ...
            vectorField centres(allCentres.size(), vector::zero_);
        ...
        )
        {
            ...
            forAll(meshPoints, pointI)
            {
                ...
                if (!glPoint)
                {
                    ...
                    {
                        // ZT: check only local processor patch cells
                        if (findIndex(localPatchCells, curCells[cellI]) == -1)
                    ...
                    // ZT: This bug-fix is not required any more since problem
                    // problem is solved by checking only local processor patch cells
                    // (see above)
                    // FIX: Ph 5-5-2021
                    ...
                    // won't b available because it doesn't contain the point)
                    // if
                    // (
                    //     curCells.size()
                    //  !=
...
void newLeastSquaresVolPointInterpolation::makeOrigins() const
{
    ...
    originsPtr_.set(new vectorField(mesh().points().size(), vector::zero_));
...
makeMirrorPlaneTransformation() const
{
    ...
    (
        ...
        (
            mesh().points().size(),
            Tuple2<vector, tensor>(vector::zero_, tensor::zero_)
        )
    );
