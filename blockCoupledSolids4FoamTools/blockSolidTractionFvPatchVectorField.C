...
) const
{
    ...
    vectorField pForce(fpatch.size(), vector::zero_);
    ...
    forAll(fpatch, faceI)
    {
        ...
        if (nonOrthogonalMesh)
        {
            ...
            //faceKPtr = new vector(vector::zero_);
            faceKPtr =
                new vector
                (
        ...
        forAll(curFaceEdges, edgeI)
        {
            ...
            forAll(sePointIDs, pointI)
            {
                ...
                symmTensor sePointFixedDir = symmTensor::zero_;
                vector sePointFixedComp = vector::zero_;
