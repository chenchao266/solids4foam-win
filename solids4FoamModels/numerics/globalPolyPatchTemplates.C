...
) const
{
    ...
    (
        new Field<Type>(globalPatch().nPoints(), pTraits<Type>::zero_)
    );
...
) const
{
    ...
    (
        new Field<Type>(patch().nPoints(), pTraits<Type>::zero_)
    );
#ifdef OPENFOAMESIORFOUNDATION
...
) const
{
    ...
    tmp<Field<Type> > tgField
    (
        new Field<Type>(globalPatch().size(), pTraits<Type>::zero_)
    );
#ifdef OPENFOAMESIORFOUNDATION
    Field<Type>& gField = tgField.ref();
#else
...
) const
{
    ...
    tmp<Field<Type> > tpField
    (
        new Field<Type>(patch().size(), pTraits<Type>::zero_)
    );
#ifdef OPENFOAMESIORFOUNDATION
    Field<Type>& pField = tpField.ref();
#else
