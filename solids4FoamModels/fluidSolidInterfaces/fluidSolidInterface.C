...
) const
{
    ...
    vectorField S(localFaces.size(), vector::zero_);
    forAll(S, faceI)
    {
...
void Foam::fluidSolidInterface::initializeFields()
{
    ...
    forAll(fluid().globalPatches(), interfaceI)
    {
        ...
        fluidZonesPointsDispls_[interfaceI] = vectorField(nPoints, vector::zero_);

        fluidZonesPointsDisplsRef_[interfaceI] =
            vectorField(nPoints, vector::zero_);
    ...
    else
    {
        ...
        else if (rbfMotionSolver)
        {
            ...
            forAll(fluidMesh().boundaryMesh(), patchI)
            {
                ...
                (
                    fluidMesh().boundaryMesh()[patchI].size(), vector::zero_
                );
            }
...
void Foam::fluidSolidInterface::updateForce()
{
    ...
    for (label interfaceI = 0; interfaceI < nGlobalPatches_; interfaceI++)
    {
        ...
        // from the fluid zone
        vectorField solidZoneTotalTraction(solidZone.size(), vector::zero_);
...
void Foam::fluidSolidInterface::updateViscousForceAndPressure()
{
    ...
    for (label interfaceI = 0; interfaceI < nGlobalPatches_; interfaceI++)
    {
        ...
        vectorField solidZoneTraction(solidZone.size(), vector::zero_);
...
Foam::scalar Foam::fluidSolidInterface::updateResidual()
{
    ...
    for (label interfaceI = 0; interfaceI < nGlobalPatches_; interfaceI++)
    {
        ...
        (
            solidZonesPointsDispls()[interfaceI].size(), vector::zero_
        );
        ...
        else
        {
            ...
            (
                fluidZonesPointsDispls()[interfaceI].size(), vector::zero_
            );
...
void Foam::fluidSolidInterface::updateMovingWallPressureAcceleration()
{
    forAll(fluid().globalPatches(), interfaceI)
    {
        ...
        )
        {
            ...
            vectorField fluidZoneAcceleration(fluidZone.size(), vector::zero_);
...
)
{
    ...
    if (Pstream::parRun())
    {
        for (label interfaceI = 0; interfaceI < nGlobalPatches_; interfaceI++)
        {
            if (!Pstream::master())
            {
                fluidZonesPointsDispls[interfaceI] = vector::zero_;
            }
