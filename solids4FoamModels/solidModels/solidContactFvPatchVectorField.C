...
:
    ...
    regionOfInterestBottomCorner_(vector::min_),
    regionOfInterest_(vector::min_, vector::max_),
#endif
    contact_(0),
    contactPerShadow_(),
...
)
{
    if (debug)
    {
        ...
        (
            ...
            "    const fvPatchFieldMapper& m\n"
            ")"
        )   << nl << "autoMap: patch = " << patch().name() << endl;
    }
...
Foam::solidContactFvPatchVectorField::normalModelForThisSlave()
{
    if (master_)
    {
        ...
        (
            "normalContactModel&"
            "solidContactFvPatchVectorField::normalModelForThisSlave()"
...
Foam::solidContactFvPatchVectorField::frictionModelForThisSlave()
{
    if (master_)
    {
        ...
        (
            ...
            "solidContactFvPatchVectorField::frictionModelForThisSlave()"
...
void Foam::solidContactFvPatchVectorField::updateCoeffs()
{
    ...
    if (master_)
    {
        ...
        forAll(shadPatchNames, shadPatchI)
        {
            ...
            vectorField patchDD(patch().size(), vector::zero_);
            ...
            else
            {
                ...
                // Avoid updating master if it is rigid
                if (!rigidMaster_)
                {
                    ...
                    // Interpolate the slave displacement increment to the
                    // master patch. This is required in order to simultaneously
                    // calculate friction traction on both contact patches
...
Foam::solidContactFvPatchVectorField::frictionHeatRate() const
{
    ...
    if (!master_)
    {
        ...
        (
            ...
            "solidContactFvPatchVectorField::frictionHeatRate() const"
    ...
    vectorField curTraction(patch().size(), vector::zero_);
...
void Foam::solidContactFvPatchVectorField::write(Ostream& os) const
{
    ...
    if (writePointDistanceFields_ && master())
    {
        ...
        (
            ...
            pMesh,
            dimensionedVector("zero", dimless, vector::zero_)
        );
#endif
