...
:
    ...
    dict_(dict),
    patch_(patch),
    displacement_(dict_.lookupOrDefault<vector>("displacement", vector::zero_)),
    ...
    currentAxisDisplacement_(vector::zero_),
    currentAngle_(0.0),
    currentRpm_(0.0),
    origPoints_(patch.points()),
    origFaceCentres_(patch.faceCentres()),
    oldPointDisp_(patch.nPoints(), vector::zero_),
    oldFaceDisp_(patch.size(), vector::zero_),
    initialRotationOrigin_(vector::zero_),
    faceDisplacementIncrement_(patch.size(), vector::zero_)
{
...
Foam::tmp<Foam::pointField> Foam::triSurfaceMotion::newPoints()
{
    ...
    vectorField pointDisp(newPoints.size(), vector::zero_);
    faceDisplacementIncrement_ = vector::zero_;
