...
void Foam::solidModel::checkWedges() const
{
    ...
    vector wedgeDirVec = vector::zero_;
    ...
    forAll(mesh.boundaryMesh(), patchI)
    {
        if (isA<wedgePolyPatch>(mesh.boundaryMesh()[patchI]))
        {
            ...
            vector firstFaceNOnMasterProc = vector::zero_;
...
void Foam::solidModel::setCellDisps(fvVectorMatrix& DEqn)
{
    ...
    if (incremental())
    {
        ...
        vectorField cellIncrDisps(cellDisps.size(), vector::zero_);
...
void Foam::solidModel::makeRhoD2dt2D() const
{
    ...
    (
        ...
        (
            ...
            mesh(),
            dimensionedVector("zero", dimForce/dimVolume, vector::zero_)
        )
    );
}
...
:
    ...
    (
        ...
        mesh(),
        dimensionedVector("0", dimLength/dimTime, vector::zero_)
    ),
    ...
    (
        ...
        pMesh_,
        dimensionedVector("0", dimLength, vector::zero_)
    ),
    pointDD_
    (
        IOobject
