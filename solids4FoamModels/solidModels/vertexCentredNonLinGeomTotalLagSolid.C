...
)
{
    ...
    forAll(dualTraction.boundaryField(), patchI)
    {
        ...
        {
            dualTraction.boundaryFieldRef()[patchI] = vector::zero_;
        }
    }
...
) const
{
    ...
    forAll(pointD.boundaryField(), patchI)
    {
        ...
        )
        {
            ...
            (
                dualMesh.boundaryMesh()[patchI].size(), vector::zero_
            );
...
)
{
    ...
    vectorField source(mesh().nPoints(), vector::zero_);
    pointD.correctBoundaryConditions();
...
bool vertexCentredNonLinGeomTotalLagSolid::evolveImplicitCoupled()
{
    ...
    vectorField pointDcorr(pointD().internalField().size(), vector::zero_);
...
bool vertexCentredNonLinGeomTotalLagSolid::evolveImplicitSegregated()
{
    ...
    vectorField pointDcorrVec(pointD().internalField().size(), vector::zero_);
        ...
        forAll(mesh().solutionD(), dirI)
        {
            ...
            if (Switch(solidModelDict().lookup("usePETSc")))
            {
                ...
                solverPerf.finalResidual() =
                    solverScalarPerf.nIterations()*vector::one_;
...
:
    ...
    fixedDofValues_(fixedDofs_.size(), vector::zero_),
    fixedDofDirections_(fixedDofs_.size(), symmTensor::zero_),
    fixedDofScale_
    (
        solidModelDict().lookupOrDefault<scalar>
        (
    ...
    (
        ...
        pMesh(),
        dimensionedVector("0", dimVelocity, vector::zero_)
    ),
    pointA_
    (
        IOobject
        ...
        pMesh(),
        dimensionedVector("0", dimVelocity/dimTime, vector::zero_)
    ),
    pointRho_
    (
        IOobject
    ...
    (
        ...
        pMesh(),
        dimensionedVector("0", dimForce/dimVolume, vector::zero_)
    ),
    dualGradDf_
    (
        IOobject
        ...
        dualMesh(),
        dimensionedTensor("zero", dimless, tensor::zero_),
        "calculated"
    ),
    dualSigmaf_
    (
        ...
        dualMesh(),
        dimensionedSymmTensor("zero", dimPressure, symmTensor::zero_),
        "calculated"
    ),
    dualFf_
    (
...
void vertexCentredNonLinGeomTotalLagSolid::writeFields(const Time& runTime)
{
    ...
    (
        ...
        pMesh(),
        dimensionedSymmTensor("0", dimless, symmTensor::zero_)
    );
