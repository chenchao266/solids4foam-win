...
void Foam::setCellDisplacements::readDict()
{
    ...
    constantDisps_.setSize(cellDispNames.size(), vector::zero_);
    timeVaryingDisps_.setSize
    (
        cellDispNames.size(), interpolationTable<vector>()
    );
    currentCellDisps_.setSize(cellDispNames.size(), vector::zero_);
    ...
    forAll(cellDispNames, cellNameI)
    {
        ...
        {
            timeVaryingDisps_[cellNameI] = interpolationTable<vector>
            (
                curCellDict.subDict("timeVaryingDisplacement")
            );
        }
        else
        {
    ...
    if (Pstream::parRun())
    {
        ...
        vectorField newConstantDisps(i, vector::zero_);
        List< interpolationTable<vector> > newTimeVaryingDisps
        (
            i, interpolationTable<vector>()
        );
        vectorField newCurrentCellDisps(i, vector::zero_);
        ...
        forAll(cellIDs_, cI)
        {
            if (cellIDs_[cI] != -1)
            {
                ...
                newTimeVaryingDisps[i] = timeVaryingDisps_[cI];
...
const Foam::vectorField& Foam::setCellDisplacements::cellDisps() const
{
    ...
    if (curTimeIndex_ != mesh_.time().timeIndex())
    {
        ...
        forAll(cellIDs_, cI)
        {
            ...
            if (curCellID != -1)
            {
                if (timeVaryingDisps_[cI].size())
                {
                    // Time-varying
                    currentCellDisps_[cI] = timeVaryingDisps_[cI]
                    (
