...
) const
{
    ...
    baseMeshField =
        dimensioned<Type>
        (
            "zero", baseMeshField.dimensions(), pTraits<Type>::zero_
        );
    ...
    forAll(baseMeshField.boundaryField(), patchI)
    {
        ...
        if (baseMeshFieldP.type() == processorFvsPatchField<Type>::typeName)
        {
            ...
            Field<Type> ngbPatchField(procPatch.size(), pTraits<Type>::zero_);
    ...
    if (nGlobalPoints)
    {
        ...
        for (label k = 0; k < nGlobalPoints; k++)
        {
            ...
            if (curSpIndex != -1)
            {
                ...
                List<vector> matAvg(subMeshes().size(), vector::zero_);
        ...
        forAll(baseMeshField.boundaryField(), patchI)
        {
            ...
            )
            {
                if (Pstream::parRun())
                {
                    ...
                    (
                        new vectorField(procPatchDispl.size(), vector::zero_)
                    );
#ifdef OPENFOAMESIORFOUNDATION
