...
:
    ...
    dict_(),
    totRefValue_(p.size(), vector::zero_),
    cohesiveLawPtr_(),
    ...
    curTimeIndex_(-1),
    initiationTraction_(p.size(), vector::zero_),
...
label simpleCohesiveZoneFvPatchVectorField::updateCrack()
{
    ...
    vectorField newTraction(patch().size(), vector::zero_);
    ...
    forAll(facesToBreak, fI)
    {
        ...
        valueFrac[faceID] = symmTensor::zero_;
...
void simpleCohesiveZoneFvPatchVectorField::updateCoeffs()
{
    ...
    vectorField curD(patch().size(), vector::zero_);
    ...
    forAll(curNormalTraction, faceI)
    {
        vector cohesiveTraction = vector::zero_;
...
void simpleCohesiveZoneFvPatchVectorField::write(Ostream& os) const
{
    ...
    damageIndicator_.writeEntry("damageIndicator", os);
