...
{
    cracked_.autoMap(m);
    tractionN_.autoMap(m);
    oldTractionN_.autoMap(m);
    tractionS_.autoMap(m);
    ...
    return
        ...
        (
            ...
            (
                ...
                Foam::sqrt
                (
                    pow(normalTrac/sigmaMax_, 2.0) + pow(shearTrac/tauMax_, 2.0)
                )
            )
        );
}
...
void Foam::variableMixedModeCohesiveZoneModel::write(Ostream& os) const
{
    ...
    os.writeKeyword("tauMax")
        << tauMax_ << token::END_STATEMENT << nl;
    os.writeKeyword("GIc")
        << GIc_ << token::END_STATEMENT << nl;
    os.writeKeyword("GIIc")
        << GIIc_ << token::END_STATEMENT << nl;

    cracked_.writeEntry("cracked", os);

    tractionN_.writeEntry("tractionN", os);
    tractionS_.writeEntry("tractionS", os);
    ...
    GI_.writeEntry("GI", os);
    GII_.writeEntry("GII", os);
