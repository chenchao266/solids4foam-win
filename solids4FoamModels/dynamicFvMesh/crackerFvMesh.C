...
void Foam::crackerFvMesh::makeGlobalCrackFaceCentresAndSizes() const
{
    ...
    globalCrackFaceCentresPtr_ =
        new vectorField(globalCrackSize, vector::zero_);
...
void Foam::crackerFvMesh::makeFaceBreakerLaw() const
{
    if (lawPtr_.valid())
    {
        FatalErrorIn("void Foam::crackerFvMesh::makeFaceBreakerLaw()")
            << "pointer already set" << abort(FatalError);
    }

    lawPtr_ = faceBreakerLaw::New("law", *this, dict_.subDict("law"));
}
...
) const
{
    ...
    for (label i = 0; i < cohesivePatchSize; i++)
    {
        ...
        )
        {
            ...
            field.boundaryField()[cohesivePatchID][i] -= 1e-12*faceN;
