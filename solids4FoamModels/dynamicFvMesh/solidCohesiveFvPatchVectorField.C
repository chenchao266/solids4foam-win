...
:
    ...
    traction_(p.size(), vector::zero_),
    pressure_(p.size(), 0.0),
    delta_(p.size(), vector::zero_),
    ...
    dict_(dict)
{
    ...
    {
        vectorField::operator=(vector::zero_);
    }
...
)
{
    ...
    if (nNewFaces > 0)
    {
        ...
        if (patchSize == 1 && nNewFaces == 1)
        {
            ...
            traction_[i] = vector::zero_;
            pressure_[i] = 0.0;
            delta_[i] = vector::zero_;
        }
...
void solidCohesiveFvPatchVectorField::write(Ostream& os) const
{
    ...
    os.writeKeyword("relaxationFactorTraction")
        << relaxationFactorTrac_ << token::END_STATEMENT << nl;
