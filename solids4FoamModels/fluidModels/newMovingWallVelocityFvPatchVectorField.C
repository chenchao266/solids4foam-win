...
#endif
    Fc_(p.patch().size(), vector::zero_),
    oldFc_(p.patch().size(), vector::zero_),
    oldoldFc_(p.patch().size(), vector::zero_),
    acceleration_(p.patch().size(), vector::zero_)
{}
...
//     Info << "newMovingWallVelocityFvPatchVectorField::updateCoeffs" << endl;
    ...
    vectorField Up(p.size(), vector::zero_);
...
snGrad() const
{
    ...
    if (db().foundObject<volTensorField>("grad(" + UName + ")"))
    {
        ...
        (
            new vectorField(this->patch().size(), vector::zero_)
        );
