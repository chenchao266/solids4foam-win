...
//     const vectorField faceCentres(mesh.faceCentres());
    ...
    const GeometricField<Type, Foam::fvPatchField, volMesh>& phi =
        mesh.lookupObject<GeometricField<Type, Foam::fvPatchField, volMesh> >
        (
            fieldName
        );
    ...
    Field<Type> patchPhi(this->patch().size(), pTraits<Type>::zero_);
...
//         Info << det(lsM) << endl;
        ...
        Field<Type> coeffs(nCoeffs, pTraits<Type>::zero_);
        Field<Type> source(iPoints_[faceI].size(), pTraits<Type>::zero_);
...
//         vector dr = C[faceI] - origin;
    ...
    if (zeroGradient_)
    {
        this->gradient() = pTraits<Type>::zero_;
    }
    else
    {
        this->gradient() =
