...
(
    ...
    fixedPatchIDs(fixedPatches.size()),
#endif
    newPoints(mesh.points().size(), vector::zero_),
    rbf(),
...
tmp<pointField> RBFMeshMotionSolver::curPoints() const
{
    ...
    (
        new vectorField(mesh().nPoints(), vector::zero_)
   );
...
void RBFMeshMotionSolver::solve()
{
    ...
    if (!rbf->rbf->computed)
    {
        ...
        vectorField positionsField(positions.rows(), vector::zero_);
    ...
    vectorField valuesField(values.rows(), vector::zero_);
    ...
    vectorField valuesInterpolationField(mesh().points().size(), Foam::vector::zero_);
    int index = 0;
    ...
    forAll(mesh().boundaryMesh(), i)
    {
        ...
        if (isFixedPatch)
        {
            for (int j = 0; j < meshPoints.size(); j++)
                valuesInterpolationField[meshPoints[j]] = Foam::vector::zero_;
        }
    }
